...
exports.generateInternSubmittedDocuments = async (req, res) => {
  ...
  try {
    ...
    if (userRole === 'adviser') {
      console.log('[generateInternSubmittedDocuments] 🔐 Adviser detected - enforcing program/year_section restrictions');
      ...
      console.log('[generateInternSubmittedDocuments] Adviser\'s program:', program, ', yearSection:', year_section);
    } else if (!['coordinator', 'superadmin'].includes(userRole)) {
      console.error('[generateInternSubmittedDocuments] ❌ User role not authorized:', userRole);
    ...
    interns.forEach((intern, index) => {
      ...
      // 📋 MOA is checked if intern's company has a MOA file (moaFile_mime_type indicates presence)
      doc.text(mark(company?.moaFile_mime_type), cols.moa.x, y + 5, { width: cols.moa.w, align: 'center' });
...
exports.getInternSubmittedDocuments = async (req, res) => {
  try {
    ...
    // Fetch interns and their documents
    const interns = await Intern.findAll({
      attributes: ['id', 'user_id', 'program', 'year_section'],
      include: [
        {
          model: User,
        ...
        },
        {
          ...
          attributes: ['id', 'document_type', 'file_name', 'uploaded_date', 'status'],
        },
      ],
    ...
    res.status(200).json({
      success: true,
      data: interns,
      message: 'Intern submitted documents retrieved successfully',
    });
  } catch (err) {
    console.error('❌ Error fetching intern documents:', err.message);
    res.status(500).json({
      success: false,
      message: 'Failed to fetch intern submitted documents',
      error: err.message,
    });
  }
};
