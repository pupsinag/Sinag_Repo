...
exports.createDailyLog = async (req, res) => {
  try {
    ...
    if (req.files && req.files.length > 0) {
      // ✅ With memory storage, files are already in buffer
      photo_content = req.files.map(f => {
        try {
          // Get file data from buffer (memory storage)
          const fileData = f.buffer;
          const base64Data = fileData.toString('base64');
          const photoObj = {
            filename: f.originalname,
            mimetype: f.mimetype,
            data: base64Data, // Convert binary to base64 string
            size: f.size,
          };
          console.log(`✅ Photo processed: ${f.originalname} (${f.size} bytes) -> base64 (${base64Data.length} chars)`);
          return photoObj;
        } catch (err) {
          console.error(`❌ Failed to process photo file ${f.originalname}:`, err.message);
          return {
            filename: f.originalname,
            mimetype: f.mimetype,
            data: null,
      ...
      console.log(`✅ Photos prepared: ${photo_content.length} photos`);
      console.log(`✅ Photo data stringified and ready for DB storage`);
      console.log(`✅ Total photo bytes to store:`, photo_content.reduce((sum, p) => sum + (p.data ? p.data.length : 0), 0));
    ...
    try {
      log = await InternDailyLog.create({
        ...
        photo_path: photo_paths, // ✅ Array of photo filenames
        photo_content: photo_content || null, // ✅ Sequelize JSON column handles stringify automatically
      });
    } catch (err) {
    ...
    console.log('   Photos stored:', photo_content ? photo_content.length : 0, 'photos');
    if (log.photo_content) {
      console.log('   ✅ photo_content field saved:', typeof log.photo_content === 'string' ? 'JSON string' : typeof log.photo_content);
    } else {
      console.log('   ⚠️ photo_content is null/undefined');
    }
    console.log('');
...
exports.getDailyLogs = async (req, res) => {
  try {
    ...
    const logs = await InternDailyLog.findAll({
      ...
      order: [['log_date', 'DESC']],
      raw: false,  // ✅ Keep as Sequelize instances to get all columns including JSON
    }).catch(async (err) => {
    ...
    // ✅ NORMALIZE photo content (parse JSON if string or Buffer)
    const normalizedLogs = logs.map((log) => {
      // Convert Sequelize instance to JSON object
      const logJSON = log.toJSON ? log.toJSON() : log;
      
      console.log(`\n📸 Log ${logJSON.id} - photo_content details:`, {
        type: typeof logJSON.photo_content,
        isBuffer: Buffer.isBuffer(logJSON.photo_content),
        isArray: Array.isArray(logJSON.photo_content),
        preview: logJSON.photo_content ? JSON.stringify(logJSON.photo_content).substring(0, 100) : 'null'
      });
      
      // ✅ Parse photo_content - handle Buffer, string, or array
      if (logJSON.photo_content) {
        let photoData = logJSON.photo_content;
        
        // 1️⃣  Convert Buffer to string
        if (Buffer.isBuffer(photoData)) {
          console.log(`🔄 Converting Buffer to string for log ${logJSON.id}`);
          photoData = photoData.toString('utf-8');
        }
        
        // 2️⃣  Parse JSON string if needed
        if (typeof photoData === 'string') {
          try {
            logJSON.photo_content = JSON.parse(photoData);
            console.log(`✅ Parsed photo_content for log ${logJSON.id} - array with ${Array.isArray(logJSON.photo_content) ? logJSON.photo_content.length : '?'} items`);
          } catch (e) {
            console.error(`❌ Failed to parse photo_content JSON for log ${logJSON.id}:`, e.message);
            logJSON.photo_content = null;
          }
        } else if (Array.isArray(photoData)) {
          logJSON.photo_content = photoData;
          console.log(`✅ photo_content is already an array with ${photoData.length} items`);
        } else {
          console.warn(`⚠️  Unexpected photo_content type for log ${logJSON.id}: ${typeof photoData}`);
          logJSON.photo_content = null;
        }
      } else {
        console.log(`ℹ️  Log ${logJSON.id} has no photo_content`);
      }

      // ❌ Remove photo_path since we're database-only
      logJSON.photo_path = undefined;
...
exports.getInternDailyLogsForAdviser = async (req, res) => {
  try {
    ...
    console.log('\n=== GET DAILY LOGS (ADVISER) ===');
      ...
      // ✅ NORMALIZE photo content (parse JSON if string or Buffer) - DATABASE ONLY
      const normalizedLogs = logs.map((instance) => {
        ...
        // ✅ Parse photo_content - handle Buffer, string, or array
        if (log.photo_content) {
          let photoData = log.photo_content;
          ...
          if (typeof photoData === 'string') {
            try {
              log.photo_content = JSON.parse(photoData);
            } catch (e) {
              console.log(`   Failed to parse photo_content JSON for log ${log.id}:`, e.message);
              log.photo_content = null;
            }
          } else if (Array.isArray(photoData)) {
            log.photo_content = photoData;
          }
        }

        // ❌ Remove photo_path since we're database-only
        log.photo_path = undefined;

        return log;
      });

      // Debug: log first 5 photo_content values
      normalizedLogs.slice(0, 5).forEach((l, idx) => {
        console.log(`   Log[${idx}] id=${l.id} photo_content=`, l.photo_content ? '(present)' : '(none)');
      });
...
exports.getCompanyInternDailyLogs = async (req, res) => {
  try {
    ...
    const normalizedLogs = logs.map((instance) => {
      // Convert Sequelize instance to plain object
      const logJSON = typeof instance.toJSON === 'function' ? instance.toJSON() : { ...instance };
      ...
      if (logJSON.photo_content) {
        ...
        if (typeof photoData === 'string') {
          try {
            logJSON.photo_content = JSON.parse(photoData);
            console.log(`✅ Parsed photo_content for log ${logJSON.id}: found ${Array.isArray(logJSON.photo_content) ? logJSON.photo_content.length : '?'} items`);
          } catch (e) {
            console.error(`❌ Failed to parse photo_content JSON for log ${logJSON.id}:`, e.message);
            logJSON.photo_content = null;
          }
...
exports.updateDailyLog = async (req, res) => {
  try {
    ...
    // Handle photo update (multiple photos) - DATABASE ONLY
    if (req.files && req.files.length > 0) {
      // ✅ With memory storage, files are already in buffer
      const photoContent = req.files.map(f => {
        try {
          const fileData = f.buffer;
          return {
            filename: f.originalname,
      ...
      log.photo_content = photoContent; // ✅ Sequelize JSON column handles stringify automatically
